...
---Returns a new stream containing the elements of the first stream followed by the elements of the rest of the streams
---A wrapper for first.concat(), but allows for streams order to be more readable
---Usage: Stream.concat(stream1, stream2, stream3) OR Stream.concat { stream1, stream2, stream3 }
---@param first Stream|Stream[]
---@param ... Stream
---@return Stream
function StreamStatic.concat(first, ...)
  -- If the first argument is a table, and it doesn't have an iter, and there are no other arguments, then we can assume it's a list of streams
  if type(first) == "table" and not first.iter then
    if select("#", ...) ~= 0 then error("Stream.concat: when using list overload, no other parameters are allowed") end
    return StreamStatic.concat(unpack(first))
  end
  return first:concat(...)
end
